...
public class RedisTool {
	...
	public static final String MUTEX_KEY = "mutexkey";
	private static final RedisTool me = new RedisTool();

	private RedisTool() {
	}

	public static RedisTool getMe() {
		return me;
	}
	...
	public boolean lock() {
		...
		try {
			...
			byte[] keyb = redisTemplate.getStringSerializer().serialize(MUTEX_KEY);
			byte[] valb = ((JdkSerializationRedisSerializer) redisTemplate.getValueSerializer()).serialize(MUTEX_KEY);
			Boolean b = connection.setNX(keyb, valb);
			LOGGER.info("add RedisLock key:{},{}", MUTEX_KEY, String.valueOf(b));
			if (b) {
				LOGGER.info("add RedisLock success");
				connection.expire(keyb, EXPIRE);
				locked = true;
				return true;
			}
		} catch (Exception e) {
			LOGGER.error("add RedisLock error", e);
		} finally {
			if (connection != null) {
				connection.close();
			}
	...
	public void unlock() {
		if (locked) {
			LOGGER.info("release RedisLock {},{}", MUTEX_KEY, get(MUTEX_KEY, String.class));
	...
	/* ----------- string --------- */
	public <T> T get(String key, TypeReference<T> typeRefer) {
		String value = redisTemplate.opsForValue().get(key);

		if (StringUtils.isEmpty(value)) {
			return null;
		}
		return JSON.parseObject(value, typeRefer);
	}
	...
	public <T> List<T> mget(String key, Class<T> clazz) {
		...
		return parseJsonArray(values, clazz);
	}
